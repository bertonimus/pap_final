...
// Variáveis da sessão
$id_utilizador = $_SESSION['id_tipos_utilizador']; // ID do utilizador logado
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['editar_produto'])) {
    ...
    if (!empty($_FILES['imagem']['name'])) {
        ...
        $caminhoImagem = "../utilizador/uploads" . basename($imagem);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['remover_produto'])) {
    ...
    if ($resultImagem->num_rows > 0) {
        ...
        $caminhoImagem = "../utilizador/uploads" . $imagem;
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
                ...
                <td>{$row['categoria']}</td>
                <td><img src='../utilizador/uploads{$row['imagem']}' alt='Imagem' width='50'></td>
                <td>
